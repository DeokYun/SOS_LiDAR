...
SOSlaser::SOSlaser()
{
  ...
  scan_.angle_increment = (PI/1800);    // 라디안 분해능  1도
...
void SOSlaser::poll()
{
   //------ 1920 * 1080 ----------//
    ...
    camMat << 1044.4, 0, 1046.6, 0, 1041.5, 651.555, 0, 0, 1;
    distCoeffs << -0.3771, 0.1174, -0.0001137, -0.00057511;
    double fx = 1044.4; double fy = 1041.5; double cx = 1046.6; double cy = 651.555;
     //------ 1920 * 1080 ----------//
      ...
      if(!tempFrame1.empty())
       {
        ...
        threshold(resized_img, bin_img, 220, 255, THRESH_TOZERO);            // 단순히 threshold로 라인레이져 잡은거 --> 다른 필터로 바꿀 수 있을듯
                                                                             // HSV로...???
        ...
        for (int c = 0; c < c_size; c++)
        {
          ...
          if (value_out[c_size-1-c] == 1)
           ...
           //------ 1920 * 1080 ----------//
            ...
            radius[c] = ((-45360/((fy* undistorted_pts.at<Vec2f>(0, c_size-1-c)[1]+cy)-657.2)+80.55)/cos(theta[c])); //for undistorted points
           //------ 1920 * 1080 ----------//
          ...
          // scan_.ranges[c] = radius[100-c]/1000.0;      // 왼쪽에서붙터라 100-c 로 바꿔줘야할듯
          scan_.ranges[c_size-1-c] = radius[c]/1000.0;
        }
        //----------------------------------------------------------------------------------------------------//

        scan_.time_increment = (1/10)/1000;   // 1sec / 28scan/ 100beam
        ...
        // ROS_INFO_STREAM("5 : " << scan_.ranges[50]);
        //-----------------------------------------------------------//
